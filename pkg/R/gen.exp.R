...
lapply( split( purchase, purchase$id ),
        ...
        # also that we make a record for the last follow-up date
        ...
        dfR$off <- (dfR$dpt==0)
        # Finally compute the cumulative dose and time on drug at the end of the interval
        # using interval length and dpt:
...
# The revised version called gen.ex
gen.exp <-
...
lapply( split( tm.dfr, tm.dfr$id ),
        ...
        dfr$off <- na.locf( dfr$off, na.rm=FALSE )
        # possible NAs are before any drug purchase
        dfr$off <- ifelse( is.na(dfr$off), FALSE, dfr$off )
        # carry date of going off drug forward, but only for the off period
        dfr$doff <- na.locf( dfr$doff, na.rm=FALSE ) * ifelse(dfr$off,1,NA)
