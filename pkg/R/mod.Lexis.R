...
mod.Lexis <- 
function( Lx, resp, formula, xpos, link="log", scale, ..., model )
{
...
#if( link=="identity" ) {
#   mll <- model( form, family = poisreg, data = Lx )
#   mod <- model( form, family = poisreg(link="identity"),
#                      mustart = fitted(mll),
#                         data = Lx, ... )
#                       } else {
...
attr( mod, "Lexis" ) <- list( Exposure=xpos, Events=resp )     
mod
}
...
glm.Lexis <- 
 function( Lx, resp, formula, xpos, link="log", scale=1    , ... ) {
mod.Lexis( Lx, resp, formula, xpos, link=link , scale=scale, ..., model=stats::glm ) }

gam.Lexis <- 
 function( Lx, resp, formula, xpos, link="log", scale=1    , ... ) {
mod.Lexis( Lx, resp, formula, xpos, link=link , scale=scale, ..., model= mgcv::gam ) }
