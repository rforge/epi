...
A timescale is a variable that varies deterministicallly \emph{within}
...
same in the two data sets; only the number of records differ.
...
@ %
A more efficient way of making this double split is to use the
\texttt{splitMulti} function from the \texttt{popEpi} package:
<<>>=
library( popEpi )
...
more efficient --- this is of particular interest if you operate on
large data sets ($>1,000,000$ records).
...
intervals. This is because the variable to be used in modeling must
be independent of the censoring and mortality pattern --- it should
only depend on the chosen grouping of the timescale.

\subsection{Difference between time scales}

However, the midpoint should be used with caution if the variable
\texttt{age1st} is modeled too; the age at hire is logically equal to
the difference between current age (\texttt{age}) and time since hire (\texttt{thf}):
<<>>=
summary( (nicS2$age-nicS2$tfh) - nicS2$age1st ) 
@ %
This calculation refer to the \emph{start} of each interval. But when
...
@ %
If all three variable are to be included in a model, you must make sure
that the \emph{substantial} relationship between the variables be
maintained. One way is to recompute age at first hire from the two
midpoint variables, but more straightforward is to use the left end
point of the intervals, that is the time scales in the \texttt{Lexis} object.
The latter approach requires that the follow-up is split in fairly
small chunks.

\section{Splitting (cutting) time at a specific date}
...
over-written by \texttt{new.state} (in this case state 2, ``High
...
Note that if the intermediate event is to be used as a time-dependent
...
the transitions between states from the \texttt{summary()}. There
...
\centering
\label{fig:nic-bos}
\includegraphics[width=0.9\textwidth]{Follow-up-nic-box}
\caption{\it Transitions between states; the number in the middle of
  each box is the person-years (in 1000s --- since \textrm{\tt
    scale.Y=1000}), the numbers at the bottom of the boxes are the
  number that start, respectively end their follow-up in each state.
  The numbers on the arrows are the number of transitions and crude
  transition rates (the latter in events per 1000 PY).\newline The
  function \textrm{\tt boxes.Lexis} has a zillion arguments to
