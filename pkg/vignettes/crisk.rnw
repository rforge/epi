...
times of the first dimension of the arrays. This is mainly for
plotting convenience.

The three components of \texttt{res} represent:
\begin{itemize}
...
The first dimension of each is time corresponding to endpoints of
intervals of length \texttt{int}, (assumed) starting at 0. The second
dimension is states (or combinations thereof). The last dimension of
the arrays is the type of statistic; \texttt{50\%} is the median of
the samples, and the bootstrap intervals as indicated.
...
str(csm) 
@ %
These are the cumulative risks evaluated at the 1001 endpoints of the
intervals, and also includes the survival probability in the first
...
\subsection{A \texttt{Lexis} object with 3 causes of death}
...
\subsection{Models for the rates}
...
%% @ %
%% \insfig{csr}{0.8}{Cause-specific mortality rates for men (full lines),
%% and women (broken lines).}
 
\subsection{Derived measures}

With these three models for the occurrence rates we can compute the
cumulative risks of dying from each of the causes. We just need a
...
\subsubsection{Cumulative risks}
...
text(0, 1/3 - 1:3/30, c("CVD","Can","Oth"), 
     col = clr, adj = 0)
@ %
\insfig{cR}{0.8}{Cumulative risks of each cause of death based on
...
\subsubsection{Stacked cumulative risks}
...
@ %
\insfig{Sr1}{0.8}{Stacked cumulative risks --- not a good graph}
It is really not a good idea to color the curves, they do not refer to
the causes of death, it is the areas \emph{between} the curves that
refer to causes. 

It would be more logical to color the areas between the
...
@ %
\insfig{Sr2}{0.8}{Stacked cumulative risks with coloring of states and
overlaid with confidence intervals for the probabilities shown; that
is the relevant sums.}

\subsubsection{Sojourn times}
...
the expected years alive is 10.9 years. The distribution of years life
lost is bogus in this case as the causes of death were randomly generated. 
