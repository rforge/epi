...
@
Each of the slices of the array \texttt{AP} is a matrix of transition
probabilities.
...
package. Note that even though there is an \texttt{etm} method for
\texttt{Lexis} objects, we must call \texttt{etm.Lexis} explicitly,
because we loaded the \texttt{etm} package \emph{after} the
\texttt{Epi} package, and thus \texttt{Epi:::etm} will be masked by
\texttt{etm:::etm}. Instead we could call \texttt{Epi:::etm}
explicitly, but that would be really goofy looking code.
