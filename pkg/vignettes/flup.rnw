...
\item[\texttt{mcutLexis}] cut follow-up at different intermediate
  events, keeping track of history
\item[\texttt{rcutLexis}] cut follow-up at intermediate events, only
  recoding the current state
...
\item[\texttt{addCov.Lexis}] add clinical measurements at a given date to a
  \texttt{Lexis} object
\item[\texttt{addDrug.Lexis}] add drug exposures to a \texttt{Lexis}
  object
\end{description}
