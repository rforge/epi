...
\texttt{Epi} package. It is intended for representation of follow-up
data (event history data) from studies where exact dates of events are
known. It accommodates follow-up through multiple states and on
...
same on all timescales. Therefore, it will suffice to use only the
...
variables that constitute the timescales:
<<>>=
...
convention \R\ that the index is taken as \emph{number of the level}
...
\texttt{split.Lexis}). This requires that the timescale and the
breakpoints on this timescale are supplied. Try:
<<>>=
...
and the follow-up may be split further along another timescale, for
...
arguments: the time point of the intermediate event, the timescale
...
diagnosis. This dataset could then be split along the timescales as we
...
Note that we defined a new timescale, \texttt{tfI}, using the argument
\texttt{new.scale=tfI}. This has a special status relative to the other
three timescales, it is defined as time since entry into a state,
namely \texttt{Ins}, this is noted in the timescale part of the
...
There is only one time-dependent here, namely \texttt{lex.Cst}, the
current state of the person's follow up; it takes the values
\texttt{DM} and \texttt{Ins} according to whether the person has ever
purchased insulin at a given time of follow-up.
...
If we want to model the effect of the timescale variables on
...
parametric functions. When we split along a timescale we can get
...
model, using the timescale as a quantitative covariate.
...
actual split times for all of the timescale variables, so these can be
...
summary( dmCs, t=T )
@ %
We see that we have 228,748 records and 9996 persons, so about 23
...
\texttt{gam.Lexis} and \texttt{coxph.Lexis}, but not elaborated here.
...
\verb|from="DM", to="Dead"|, omits modeling contributions
from the $\mathtt{Ins}\rightarrow\mathtt{Dead}$ transition. The other
three models all use both transitions --- and assume that the two
transition rates are the same, \ie that start of insulin has no effect
on mortality. We shall relax this assumption later.
...
\texttt{tfI}. But since \texttt{tfI} is a timescale defined as time
...
not give the parameters for the baseline timescale (\texttt{age}), but
...
@ %
\insfig{IeffR}{0.8}{Insulin duration effect (state \textrm{\tt Ins}) relative to no
  insulin (state \textrm{\tt DM}), black from Poisson model, red from Cox
  model. The \emph{shape} is the same as the previous figure, but the
  RR is now relative to non-insulin, instead of relative to insulin
  users at 2 years duration.}
...
Both figure \ref{fig:Ieff} and \ref{fig:IeffR} indicates a declining
...
\insfig{dur-int}{0.8}{Age at insulin as interaction between age and
  duration. Blue curves are from the naïve interaction model
...
\insfig{splint}{0.8}{Spline-by-spline interaction between age and
  duration (black), and the interaction using a non-linear effect of
  age at entry (red), corresponding to the red curves in figure
  \ref{fig:dur-int}.}
...
\insfig{RR-int}{0.8}{The effect of duration of insulin use at
  ...
  effect of age at insulin start (red) and using a general spline
  interactions (black). It appears that the general interaction
  over-models a bit.}
...
models are uncorrelated, and hence qualify for \texttt{ci.ratio}:
<<sep-HR,fig=TRUE>>=
par(mar=c(3,3,1,1),mgp=c(3,1,0)/1.6,las=1,bty="n")
...
timescales. This can be accomplished by the \texttt{mcutLexis}
...
summary( dmM, t=T )
@
We see that we now have two timescales defined as entry since into
states.
<<>>=
...
multistate data. Here is a list of functions with a very brief
description; it does not replace the documentation. Unless otherwise
stated function named \texttt{something.Lexis} (with a ``\texttt{.}'')
are S3 methods for \texttt{Lexis} objects, so you can skip the
``\texttt{.Lexis}'' in daily use.
...
\item[\texttt{splitLexis}] split follow up along a timescale
\item[\texttt{splitMulti}] split follow up along a timescale --- from
...
\item[\texttt{nid.Lexis}] number of persons in the \texttt{Lexis}
  object --- how many different values of \texttt{lex.id} are there
\item[\texttt{entry}] entry time
\item[\texttt{exit}] exit time
\item[\texttt{status}] status at entry or exit
\item[\texttt{timeBand}] factor of time band
\item[\texttt{timeScales}] what timescales are in the \texttt{Lexis} object
\item[\texttt{timeSince}] what timescales are defined as time since a given state
...
\item[\texttt{preceding}, \texttt{before}] which states precedes this
...
\item[\texttt{merge.Lexis}] merges two \texttt{Lexis} objects  
\item[\texttt{cbind.Lexis}] append a \texttt{data.frame} to a \texttt{Lexis} object
\item[\texttt{rbind.Lexis}] put two \texttt{Lexis} objects head-to-foot
\item[\texttt{transform.Lexis}] transform variables
\item[\texttt{tsNA20}] turn \texttt{NA}s to 0s for timescales
...
\begin{description}  
\item[\texttt{glm.Lexis}, \texttt{gam.Lexis}] fit \texttt{glm}
  resp. \texttt{gam} (from \texttt{mgcv}) model using the \texttt{poisreg} family to
